...
def  main():
  ...
  installer = "\installer\CqtAnalyzer Installer.exe"
   
  if demo:
    installer = "\installer\CqtAnalyzer Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\CqtAnalyzer manual.pdf" 
  ]
  ...
  ZIPNAME = "CqtAnalyzer-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "CqtAnalyzer-v" + FULLVERSIONSTR + "-win-demo.zip"
